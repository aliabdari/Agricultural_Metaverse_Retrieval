from DNNs import GRUNet, VideoModel, RoomModel, MuseumModel, CombineNetEarlyFusion
import torch
...
def start_train(args):
    ...
    model_aggregator = CombineNetEarlyFusion()
    ...
    params = list(model_desc.parameters()) + list(model_video.parameters()) + list(model_room.parameters()) \
             + list(model_museum.parameters()) + list(model_aggregator.parameters())
    ...
    for _ in tqdm(range(num_epochs)):
        ...
        for i, (data_desc, data_frame_img, data_frame_vid, data_nvr, data_r) in enumerate(train_loader):
            ...
            output_video_img = model_video(data_frame_img)

            data_frame_vid = torch.squeeze(data_frame_vid, 2)

            output_video = model_aggregator(output_video_img, data_frame_vid)
        ...
        if r10 > best_r10:
            ...
            train_utility.save_best_model(approach_name, model_video.state_dict(), model_room.state_dict(),
                                          model_museum.state_dict(), model_aggregator.state_dict(), model_desc.state_dict())

    bm_video, bm_room, bm_museum, bm_aggregator, bm_desc = train_utility.load_best_model(approach_name)
