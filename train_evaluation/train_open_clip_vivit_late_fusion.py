from DNNs import GRUNet, VideoModel, RoomModel, MuseumModel, CombineNetLateFusion
import torch
from torch.nn.utils.rnn import pad_sequence, pack_padded_sequence
from torch.utils.data import Subset, DataLoader
from torch.optim.lr_scheduler import StepLR
import train_utility
from Data_utils import DescriptionSceneCombined
...
def collate_fn(data):
    ...
    # museums
    tmp_mus_1 = [x[1] for x in data]
    videos_1 = [uniform_sample(z[:50, :]) for x in tmp_mus_1 for y in x for z in y]
    frames_1 = pad_sequence(videos_1, batch_first=True)
    frames_1 = torch.transpose(frames_1, 1, 2)

    tmp_mus_2 = [x[2] for x in data]
    videos_2 = [z for x in tmp_mus_2 for y in x for z in y]
    frames_2 = pad_sequence(videos_2, batch_first=True)
    frames_2 = torch.transpose(frames_2, 1, 2)

    # num_videos_in_room = [[len(y) for y in x] for x in tmp_mus]
    num_videos_in_room = [len(y) for x in tmp_mus_1 for y in x]
    num_rooms = [len(x) for x in tmp_mus_1]

    return descs, frames_1, frames_2, num_videos_in_room, num_rooms
...
def start_train(args):
    ...
    mode = Constants.mode_open_clip_vivit
    ...
    model_aggregator = CombineNetLateFusion()

    model_room_vid = RoomModel(in_channels=768, out_channels=512, kernel_size=3, feature_size=512)
    # model_combiner =
    ...
    model_room_vid.to(device=device)
    model_aggregator.to(device=device)
    ...
    descriptions_path, videos_path_img, videos_path_vid, museum_path = train_utility.get_entire_data(mode=mode)

    dataset = DescriptionSceneCombined(data_description_path=descriptions_path, data_video_path_img=videos_path_img,
                                       data_video_path_vid=videos_path_vid, data_museum_path=museum_path, mode=mode,
                                       mem=True)
    ...
    '''Train Procedure'''
    params = list(model_desc.parameters()) + list(model_video.parameters()) + list(model_room.parameters()) \
             + list(model_museum.parameters()) + list(model_room_vid.parameters()) + list(model_aggregator.parameters())
    ...
    for _ in tqdm(range(num_epochs)):
        ...
        for i, (data_desc, data_frame_img, data_frame_vid, data_nvr, data_r) in enumerate(train_loader):
            data_desc = data_desc.to(device)
            data_frame_img = data_frame_img.to(device)
            data_frame_vid = data_frame_vid.to(device)

            optimizer.zero_grad()

            output_desc = model_desc(data_desc)
            output_video = model_video(data_frame_img)

            input_room_model_img = prepare_input(output_data=output_video, list_nums=data_nvr)
            output_room_img = model_room(input_room_model_img, data_nvr)

            input_room_model_vid = prepare_input(output_data=data_frame_vid, list_nums=data_nvr)
            input_room_model_vid = torch.squeeze(input_room_model_vid, -1)
            output_room_vid = model_room_vid(input_room_model_vid, data_nvr)
            
            aggregation = model_aggregator(output_room_img, output_room_vid)

            input_museum_model = prepare_input(output_data=aggregation, list_nums=data_r)
            output_museum = model_museum(input_museum_model, data_r)

            # print(sum(data_nvr))
            # print(sum(data_r))
            # print(input_museum_model.shape)
            # print(output_museum.shape)
            # print(input_room_model_img.shape)
            # print(output_video.shape)
            # print(output_room_img.shape)
            # print(data_frame_vid.shape)
            # print(output_room_vid.shape)
        ...
        model_museum.eval()
        with torch.no_grad():
            for j, (data_desc, data_frame_img, data_frame_vid, data_nvr, data_r) in enumerate(val_loader):
        ...
        if r10 > best_r10:
            best_r10 = r10
            train_utility.save_best_model(approach_name, model_video.state_dict(), model_room.state_dict(),
                                          model_museum.state_dict(), model_room_vid.state_dict(),
                                          model_aggregator.state_dict(), model_desc.state_dict())

    bm_video, bm_room, bm_museum, bm_room_vid, bm_aggregator, bm_desc = train_utility.load_best_model(approach_name)
    model_video.load_state_dict(bm_video)
    model_room.load_state_dict(bm_room)
    model_museum.load_state_dict(bm_museum)
    model_room_vid.load_state_dict(bm_room_vid)
    model_aggregator.load_state_dict(bm_aggregator)
    ...
    # Evaluate test set
    with torch.no_grad():
        for j, (data_desc, data_frame_img, data_frame_vid, data_nvr, data_r) in enumerate(test_loader):
