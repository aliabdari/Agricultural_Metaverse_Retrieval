...
def collate_fn(data):
    ...
    idx_nvm = 0
    for r in num_rooms:
        num_videos_in_museum.append(sum(num_videos_in_room[idx_nvm:idx_nvm+r]))
        idx_nvm += r

    frames_in_museums = list()
    idx_fvm = 0
    for v in num_videos_in_museum:
        frames_in_museums.append(torch.concat(videos[idx_fvm:idx_fvm+v], dim=0))
        idx_fvm += v

    num_frames_museums = [x.shape[0] for x in frames_in_museums]

    frames_in_museums_tensor = pad_sequence(frames_in_museums, batch_first=True)
    frames_in_museums_tensor = torch.transpose(frames_in_museums_tensor, 1, 2)
    return descs, frames_in_museums_tensor, num_frames_museums
...
def start_train(args):
    ...
    mode = Constants.mode_open_clip_non_hierarchical_one_level
    ...
    '''Train Procedure'''
    params = list(model_desc.parameters()) + list(model_museum.parameters())
    ...
    for _ in tqdm(range(num_epochs)):
        ...
        for i, (data_desc, data_frame, data_nfm) in enumerate(tqdm(train_loader, total=(len(train_indices) // batch_size))):
            ...
            output_museum = model_museum(data_frame, data_nfm)
        ...
        model_museum.eval()
        with torch.no_grad():
            for j, (data_desc, data_frame, data_nfm) in enumerate(val_loader):
        ...
        if r10 > best_r10:
            best_r10 = r10
            train_utility.save_best_model(approach_name, model_museum.state_dict(), model_desc.state_dict())
    ...
    # Evaluate test set
    with torch.no_grad():
        for j, (data_desc, data_frame, data_nfm) in enumerate(test_loader):
