...
def collate_fn(data):
    ...
    for r in num_rooms:
        ...
        idx_nvr += r
    return descs, frames, num_videos_in_museum
...
def start_train(args):
    ...
    mode = Constants.mode_open_clip_non_hierarchical_two_level
    ...
    '''Train Procedure'''
    params = list(model_desc.parameters()) + list(model_video.parameters()) + list(model_museum.parameters())
    ...
    for _ in tqdm(range(num_epochs)):
        ...
        for i, (data_desc, data_frame, data_nvm) in enumerate(tqdm(train_loader, total=(len(train_indices) // batch_size))):
            ...
            input_museum_model = prepare_input(output_data=output_video, list_nums=data_nvm)
            output_museum = model_museum(input_museum_model, data_nvm)
        ...
        model_museum.eval()
        with torch.no_grad():
            for j, (data_desc, data_frame, data_nvm) in enumerate(val_loader):
        ...
        if r10 > best_r10:
            best_r10 = r10
            train_utility.save_best_model(approach_name, model_video.state_dict(), model_museum.state_dict(), model_desc.state_dict())

    bm_video, bm_museum, bm_desc = train_utility.load_best_model(approach_name)
    ...
    # Evaluate test set
    with torch.no_grad():
        for j, (data_desc, data_frame, data_nvm) in enumerate(test_loader):
