...
def collate_fn(data):
    ...
    videos = [z for x in tmp_mus for y in x for z in y]
...
def start_train(args):
    ...
    model_room = RoomModel(in_channels=768, out_channels=512, kernel_size=3, feature_size=512)
    ...
    dataset = DescriptionScene(data_description_path=descriptions_path, data_video_path=videos_path,
                               data_museum_path=museum_path, mem=True, mode=mode)
    ...
    for _ in tqdm(range(num_epochs)):
        ...
        for i, (data_desc, data_room, data_nvr, data_r) in enumerate(tqdm(train_loader, total=(len(train_indices) // batch_size))):
            data_desc = data_desc.to(device)
            data_room = data_room.to(device)

            optimizer.zero_grad()

            output_desc = model_desc(data_desc)
            input_room_model = prepare_input(output_data=data_room, list_nums=data_nvr)
            input_room_model = torch.squeeze(input_room_model, -1)
        ...
        model_museum.eval()
        with torch.no_grad():
            for j, (data_desc, data_room, data_nvr, data_r) in enumerate(val_loader):
    ...
    # Evaluate test set
    with torch.no_grad():
        for j, (data_desc, data_room, data_nvr, data_r) in enumerate(test_loader):
