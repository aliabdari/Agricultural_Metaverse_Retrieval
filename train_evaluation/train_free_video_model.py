...
def start_process():
    ...
    fvrs = [('Mean', 'Mean'), ('Median', 'Mean')]

    path_tensors_videos = '../features/clip4clip_features/videos'
        ...
        for idx_m in test_indices:
            ...
            for idx_r, r in enumerate(m['rooms']):
                ...
                for idx_v, v in enumerate(m['rooms'][r]):
                    features_videos = torch.load(path_tensors_videos + os.sep + v + '.pt', weights_only=True)
                    features_frames_rooms[idx_v, :] = features_videos
                features_frames_museum[idx_r, :] = aggregation_func(fvr[0], features_frames_rooms)
                # features_descriptions_museum.extend(features_descriptions_rooms)
            total_museums_frames_features.append(aggregation_func(fvr[1], features_frames_museum))
