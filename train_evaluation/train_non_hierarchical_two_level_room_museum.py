...
def collate_fn(data):
    ...
    frames_in_rooms = list()
    idx_fvm = 0
    for v in num_videos_in_room:
        frames_in_rooms.append(torch.concat(videos[idx_fvm:idx_fvm + v], dim=0))
        idx_fvm += v

    num_frames_rooms = [x.shape[0] for x in frames_in_rooms]

    frames_in_rooms_tensor = pad_sequence(frames_in_rooms, batch_first=True)
    frames_in_rooms_tensor = torch.transpose(frames_in_rooms_tensor, 1, 2)

    # num_videos_in_museum = list()
    # idx_nvr = 0
    # for r in num_rooms:
    #     num_videos_in_museum.append(sum(num_videos_in_room[idx_nvr:idx_nvr+r]))
    #     idx_nvr += r
    return descs, frames_in_rooms_tensor, num_frames_rooms, num_rooms
...
def start_train(args):
    ...
    mode = Constants.mode_open_clip_non_hierarchical_two_level_room_museum
    ...
    '''Train Procedure'''
    params = list(model_desc.parameters()) + list(model_room.parameters()) + list(model_museum.parameters())
    ...
    for _ in tqdm(range(num_epochs)):
        ...
        for i, (data_desc, data_frame, data_nfr, data_r) in enumerate(tqdm(train_loader, total=(len(train_indices) // batch_size))):
            ...
            output_room = model_room(data_frame, data_nfr)
        ...
        model_museum.eval()
        with torch.no_grad():
            for j, (data_desc, data_frame, data_nfr, data_r) in enumerate(val_loader):
        ...
        if r10 > best_r10:
            best_r10 = r10
            train_utility.save_best_model(approach_name, model_room.state_dict(), model_museum.state_dict(), model_desc.state_dict())

    bm_room, bm_museum, bm_desc = train_utility.load_best_model(approach_name)
    ...
    # Evaluate test set
    with torch.no_grad():
        for j, (data_desc, data_frame, data_nfr, data_r) in enumerate(test_loader):
