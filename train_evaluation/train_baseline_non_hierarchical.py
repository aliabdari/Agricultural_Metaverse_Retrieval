from DNNs import GRUNet, VideoModel, RoomModel, MuseumModel, OneDimensionalCNN
import torch
...
def collate_fn(data):
    ...
    # videos = [z[:50, :] for x in tmp_mus for y in x for z in y]
    tmp_mus_ = []
    num_videos = []
    for x in tmp_mus:
        ff = []
        for y in x:
            for z in y:
                ff.append(torch.mean(z[:50, :], 0))
        tmp_mus_.append(torch.stack(ff))
        num_videos.append(len(ff))
    padded_video = pad_sequence(tmp_mus_, batch_first=True)
    frames = torch.transpose(padded_video, 1, 2)
    # videos = [torch.mean(z[:50, :], 0) for x in tmp_mus for y in x for z in y]
    ...
    return descs, frames, num_videos, None
...
def start_train(args):
    ...
    mode = Constants.mode_open_clip

    approach_name = Constants.base_approach
    ...
    # model_video = VideoModel(in_channels=512, out_channels=512, kernel_size=3, feature_size=256)
    model_visual = OneDimensionalCNN(in_channels=512, out_channels=512, kernel_size=3, feature_size=256)
    ...
    '''Train Procedure'''
    params = list(model_desc.parameters()) + list(model_visual.parameters())
    ...
    for _ in tqdm(range(num_epochs)):
        ...
        for i, (data_desc, data_frame, num_videos, data_r) in enumerate(train_loader):
            ...
            output_visual = model_visual(data_frame, num_videos)

            print(data_frame.shape)
            exit(0)

            multiplication_dp = train_utility.cosine_sim(output_desc, output_visual)
        ...
        model_visual.eval()
        with torch.no_grad():
            for j, (data_desc, data_frame, num_videos, data_r) in enumerate(val_loader):
                ...
                output_museum_val[initial_index:final_index, :] = output_visual
        ...
        if r10 > best_r10:
            best_r10 = r10
            train_utility.save_best_model(approach_name, model_visual.state_dict(), model_desc.state_dict())

    bm_museum, bm_desc = train_utility.load_best_model(approach_name)
    model_visual.load_state_dict(bm_museum)
    ...
    # Evaluate test set
    with torch.no_grad():
        for j, (data_desc, data_frame, num_videos, data_r) in enumerate(test_loader):
            ...
            output_museum_test[initial_index:final_index, :] = output_visual
